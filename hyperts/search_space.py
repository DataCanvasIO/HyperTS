...
def search_space_univariate_forecast_generator(covariate=(), timestamp=None):
    ...
    def f(dataframe_mapper_default=False, impute_strategy='mean', seq_no=0):
        ...
        with space.as_default():
            ...
            TSEstimatorMS(ProphetWrapper, fit_kwargs, interval_width=Choice([0.5, 0.6]), seasonality_mode=Choice(['additive', 'multiplicative']))(last_transformer)
    ...
    def f(dataframe_mapper_default=False, impute_strategy='mean', seq_no=0):
        ...
        with space.as_default():
            ...
            if len(covariate) > 0 or timestamp is not None:
                ...
                last_transformer = input
            TSEstimatorMS(VARWrapper, fit_kwargs, ic=Choice(['aic', 'fpe', 'hqic', 'bic']))(last_transformer)
            space.set_inputs(input)
        return space
    return f
